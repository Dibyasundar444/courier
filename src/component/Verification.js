...
function Verification({ navigation, route }) {
  ...
  const submitHandler=()=>{
    axios.post(`${API}/activation`,{"otpcode":otp})
    .then((resp)=>{
      if(resp.status === 200){
    ...
    <View style={styles.container}>
      ...
      <View style={{backgroundColor:"#fff",flex:1,borderTopLeftRadius:30}}>
        <View style={{marginVertical:20,marginHorizontal:20}}>
          <Text style={{fontSize:16}}>Enter 6-digit verification code sent to</Text>
          <Text style={{fontSize:16,color:"#e0ab24"}}>{email}</Text>
          <View style={{marginTop:80}}>
            <Text style={{fontSize:18,fontWeight:"700"}}>Enter OTP</Text>
            <TextInput 
              placeholder="Enter 6-digit verification code"
              style={{borderBottomWidth: 0.5,marginTop:10,fontSize:18}}
              value={otp}
